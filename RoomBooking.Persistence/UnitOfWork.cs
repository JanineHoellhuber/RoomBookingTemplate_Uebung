...
namespace RoomBooking.Persistence
{
  public class UnitOfWork : IUnitOfWork
  {
    ...
    private async Task ValidateEntityAsync(object entity)
    {
      ...
      {
        if(await _dbContext.Customers.AnyAsync(c => c.Id != customer.Id && c.FirstName != customer.FirstName && c.LastName != customer.LastName && c.Iban != customer.Iban))
        {
                    throw new ValidationException($"Customer {customer.FirstName} {customer.LastName} ist bereits vorhanden.");
        }
      }
